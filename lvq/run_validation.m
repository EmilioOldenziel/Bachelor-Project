...
function [gmlvq_mean, roc_validation, lcurves_mean,...
          lcurves_sdt, param_set,f_measures] = ...
...
% up to and including version 2.2:   wm= squeeze(protos(1,:,:)); wm2=wm.^2;
% wrong initialization of averages produces stretched (mean prototypes)

% corrected from version 2.3 onward: 
wm= squeeze(protos(1,:,:))/nruns; wm2=nruns*wm.^2; 
...
for ipt=2:nruns;
 ...
 % define structures for output
   ...
   param_set = struct('totalsteps',totalsteps,'doztr',...
    ...
    f_measures = struct('f_micro_average',mean(f_micro_average),...
                        'f_macro_average', mean(f_macro_average));
